...
var checkService = function(name, uart) {
  ...
  if (Object.keys(uart).length === 0) {
    ...
    console.log(
      "Expecting service name to be one of: " +
        Object.keys(knownUartServices).join(", ") +
        "."
    );
    console.log(
      "Or pass serviceUUID, txUUID, & rxUUID in object literal as second argument."
    );
    error = true;
  } else {
    if (!uart.serviceUUID) {
...
var BleUart = function(deviceName, name, options) {
  ...
  function scan(state) {
    if (state === "poweredOn") {
      // if the radio's on, scan for this service
